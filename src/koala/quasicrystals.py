...
def penrose_tiling(number_of_db_lines: int) -> Lattice:
    ...
    lattice = de_brujin_grid(number_of_db_lines,number_of_bundles,grid_offsets)
    
    return lattice


def de_brujin_grid(number_of_lines: int, number_of_bundles = 5, grid_offsets = None, angle_disorder = 0) -> Lattice:
    ...
    angles = np.arange(number_of_bundles)*(2*np.pi/number_of_bundles) + angle_disorder*(np.random.random(number_of_bundles)-0.5)*2*np.pi
    ...
    number_of_vertices = (number_of_lines*number_of_lines*(number_of_bundles-1)*number_of_bundles)//2

    # find the vertices
    all_vertices = np.zeros([number_of_vertices, 2])                # positions
    all_bundle_intersections = np.zeros([number_of_vertices, 2])    # index of the bundles that make the vertex
    all_line_intersections = np.zeros([number_of_vertices, 2])      # index of the line for this intersection
    all_line_positions = np.zeros([number_of_vertices, 2])          # value of \mu parameter on each line for the intersection
    ...
    # find the edges
    number_edges = number_of_bundles*number_of_lines*((number_of_bundles-1)*number_of_lines-1)
    ...
    for b in range(number_of_bundles):
        ...
        for l in range(number_of_lines):
            ...
            edge_bundles[insertion_point: insertion_point+ new_edges.shape[0]] = b
