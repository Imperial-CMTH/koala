...
def tri_square_pent():
    ...
    edge_indices = np.array(
        [[0, 1], [0, 2], [1, 2], [1, 3], [2, 4], [3, 4], [3, 5], [5, 6], [6, 7], [4, 7]]
    )
...
def generate_honeycomb(n_horizontal_cells: int, return_coloring=False) -> Lattice:
    ...
    crossing_hor[
        np.arange(n_vertical) * n_horizontal_cells + (n_horizontal_cells - 1), 0
    ] = 1
    crossing_ver = np.zeros_like(ext_ver)
    crossing_ver[
        np.arange(
            n_horizontal_cells * (n_vertical - 1), n_horizontal_cells * n_vertical
        ),
        1,
    ] = -1
    ...
    coloring = np.array(
        [0, 2, 0] * n_vertical * n_horizontal_cells
...
def generate_hex_square_oct(n_cells: int) -> Lattice:
    ...
    site_coordinates = np.array(
        [[0.5, 0.17], [0.2, 0.35], [0.2, 0.65], [0.5, 0.82], [0.8, 0.65], [0.8, 0.35]]
    )
    ...
    ext_ver = np.array(
        [
            [6 * next_direction(n_cells, n, [0, 1]), 3 + 6 * n]
            for n in np.arange(n_cells**2)
        ]
    )
    crossing_ver = np.zeros_like(ext_ver)
    ...
    for n_position, xy_pos in enumerate(zip(x_shifts, y_shifts)):
        for n_edge, ec in enumerate(zip(unit_edges, unit_crossing)):
            ...
            all_edges[n_index] = [
                ...
                edge[1]
                + n_internal_sites * _next_cell_number(nx, ny, n_position, crossing),
            ]
...
def make_amorphous(
    ...
    lattice = voronization.generate_lattice(points, shift_vertices=shift_vertices)
    ...
    gs_flux_sector = np.array(
        [ground_state_ansatz(p.n_sides) for p in lattice.plaquettes], dtype=np.int8
    )
