...
def _process_plot_args(lattice, ax, labels, color_scheme, subset, N, kwargs):
    ...
    if not hasattr(Lattice, "unit_cell"):
        Lattice.unit_cell = matplotlib.transforms.IdentityTransform()
...
def plot_edge_indices(lattice, ax=None, offset=0.01):
    ...
    for i, e in enumerate(lattice.edges.indices):
        midpoint = lattice.vertices.positions[e[0]] + lattice.edges.vectors[i] / 2
        midpoint = midpoint % 1  # wrap around the unit cell
        ax.text(*(midpoint + offset), f"{i}", color="g",ha='right', va='center')
...
def peru_friendly_color_scheme(n_colors: int):
    if n_colors == 2:
        ...
        raise LatticeException(
            "Honestly you cannot possibly expect someone as colorblind as me to be able to find FIVE colors I can tell apart, thats ridiculous"
        )
...
def plot_lattice(
    ...
    vertices, adjacency, adjacency_crossing = (
        lattice.vertices,
        ...
        for i, color, (start, end), bond_sign in zip(
            ...
            if edge_index_labels[i]:
                ax.text(
                    *(center),
                    f"{i}",
                    color="w" if edge_arrows[i] else "k",
                    ha="center",
                    va="center",
                    zorder=5,
                )
    ...
    if (scatter_args is not None) or (vertex_labels is not None):
        ax.scatter(
...
def plot_degeneracy_breaking(vertex_i, g, ax=None):
    ...
    plot_lattice(
        g,
        edge_labels=highlight_edge_neighbours,
        scatter_args=dict(color=vertex_colors),
        ax=ax,
    )
...
def line_intersection(lines, lines2, abs_tolerance=1e-14, full_output=False):
    ...
    if full_output:
        return intersect, are_parallel, are_colinear
    return intersect
