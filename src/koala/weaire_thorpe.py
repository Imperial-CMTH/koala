...
def vertices_to_triangles(g, edge_labels, triangle_size=0.05):
    ...
    """
    new_vertices = np.zeros(shape=(g.vertices.positions.shape[0] * 3, 2),
                            dtype=float)
    new_adjacency = np.zeros(shape=(g.edges.indices.shape[0] +
                                    g.vertices.positions.shape[0] * 3, 2),
                             dtype=int)
    new_adjacency_crossing = np.zeros(
        shape=(g.edges.indices.shape[0] + g.vertices.positions.shape[0] * 3, 2),
        dtype=int)
    ...
    for vertex_i in range(g.vertices.positions.shape[0]):
        ...
        for k, vertex_j, edge_j in zip(it.count(), vertex_indices,
                                       edge_indices):
            ...
            this_triangle_size = min(0.2 * np.linalg.norm(edge_vectors[k]),
                                     triangle_size)
            new_vertex_position = (
                this_vertex + this_triangle_size * normalised(edge_vectors[k]))
            other_vertex_position = (
                this_vertex + edge_vectors[k] -
                this_triangle_size * normalised(edge_vectors[k]))
            ...
            new_adjacency_crossing[edge_j] = np.floor(
                other_vertex_position) - np.floor(new_vertex_position)
            ...
            new_adjacency[g.edges.indices.shape[0] + index] = (index,
                                                               other_index)
    ...
    for edge_j in np.arange(g.edges.indices.shape[0], new_adjacency.shape[0]):
        ...
        new_adjacency_crossing[edge_j] = (np.floor(end) -
                                          np.floor(start)).astype(int)
...
def make_weire_thorpe_model(g: Lattice,
                            internal_edges: np.ndarray,
                            phi: float = 0,
                            V: float = 1,
                            W: float = 0.66) -> np.ndarray:
