...
def ujk_from_fluxes(lattice: Lattice, target_flux_sector : np.ndarray = None, 
    ...
    if initial_ujk_guess is None: initial_ujk_guess = np.ones(lattice.n_edges, dtype = np.int8)
    initial_ujk_guess, target_flux_sector = initial_ujk_guess.copy(), target_flux_sector.copy()
    ...
    bonds, fluxes_to_flip = _flip_adjacent_fluxes(lattice, initial_ujk_guess, fluxes_to_flip)
